...
export const uploadDocument = async(req: Request, res: Response, next: NextFunction) => {
    ...
    if (!userId) {
        ...
        const {documentId, filePath: documentPath} = await handleUploadDocument(documentName, file, userId)
 
        const {totalPages, chunkCount, status} = await chunkDocumentService(documentId, documentPath)
